(() => {
  ...
  function presetStateCatalog() {
    let catalog = state.presetState.catalog;
    if (!catalog || typeof catalog !== "object" || Array.isArray(catalog)) {
      catalog = normalizeCatalog(catalog);
      state.presetState.catalog = catalog;
    }
    if (state.presetState.custom !== catalog.custom) state.presetState.custom = catalog.custom;
    return catalog;
  }

  function effectivePresetRules() {
    const catalog = presetStateCatalog();
    const cached = state.effectiveRulesCache;
    if (cached
      && cached.catalog === catalog
      && cached.builtin === state.presetRules
      && cached.categories === state.presetCatalog) return cached.rules;
    const rules = presetCatalogApi().effectiveRules(catalog, state.presetRules || {}, state.presetCatalog || []);
    state.effectiveRulesCache = {
      catalog,
      builtin: state.presetRules,
  ...
  function findPresetCategory(categoryId) {
    return state.presetCatalog.find((item) => item.id === categoryId) || null;
  }
  ...
  function getSlotState(categoryId, slotId, { initialize = true } = {}) {
    const key = slotStorageKey(categoryId, slotId);
    let current = state.presetState.slots[key];
    if (!current || typeof current !== "object" || Array.isArray(current)) {
      ...
      current = state.presetState.slots[key] = {
        value: pool[0] || "",
        locked: false,
        ignored: false,
      };
  ...
  function categoryActiveTags(category) {
    ...
    for (const slot of category.slots || []) {
      const current = state.presetState.slots[slotStorageKey(category.id, slot.id)];
  ...
  function skippedCategoryIds(tags = null) {
    ...
    const rules = effectivePresetRules();
    const categories = [...state.presetCatalog];
  ...
  function collectPresetConflicts() {
    ...
    for (const category of state.presetCatalog) {
      if (skipped.has(category.id)) continue;
      for (const slot of category.slots || []) {
        const key = slotStorageKey(category.id, slot.id);
        const current = state.presetState.slots[key];
  ...
  function randomizePresetSlots(categoryId = "", { persist = true } = {}) {
    ...
    const ordered = [...state.presetCatalog].sort((left, right) => {
  ...
  function composePresetPrompt() {
    ...
    const panties = state.presetCatalog.find((category) => category.id === "panties");
    const take = (values) => {
    ...
    const extra = String(state.presetState.extraText || "").trim();
